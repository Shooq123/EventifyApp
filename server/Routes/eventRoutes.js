...
router.post("/createEvent", async (req, res) => {
  try {
    const { title, date, location, description, price, type } = req.body;
    ...
    const event = new EventModel({ title, date, location, description, price, type });
    await event.save();
...
router.post("/bookEvent", async (req, res) => {
  ...
  res.status(200).json({ message: "Event booked successfully!", eventId, userId });
});
